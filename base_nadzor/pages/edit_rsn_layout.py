...
engine_sql_lite = sqlalchemy.create_engine('sqlite:///base_nadzor/read_db/NADZO_DB.db')
...
for x in labels_full:
    if 'X' in x:
        for num_x in range(1, 6):
            labels_full.append(x.replace('X', str(num_x)))
...
for x in labels:
    if x in non_input_labels:
        labels.remove(x)
    elif 'X' in x:
        for num_x in range(1, 6):
            labels.append(x.replace('X', str(num_x)))


def edit_rsn(row: dict):
    ...
    for key, value in row.items():
        if value:
            if str(key)[0].isdigit():
                lab_ = [i for i in labels_full if i.startswith(key)][0]
                form_edit_array.append(dbc.Label(lab_))
                form_edit_array.append(dbc.Input(id=f'edit_rsn_{key.split()[0].replace(".", "_")}', value=value))

    layout = html.Div(children=[
        dbc.Button("Сохранить", color="success", className="me-1", id='save_edit2_rsn_to_db',
                   style={'margin': '10px'}),
        html.Div(children=[html.P(id='vers_edit', children=[version])], style={'display': 'none'}),
        html.Div(form_edit_array, className="mb-3")]
    )
