...
def make_layout_rns():
    layout = html.Div([
        ...
        html.Div(id='rsn_edit2_null_div')
    ], className='container')
...
for x in labels:
    if x in non_input_labels:
        ...
        y = x.split()[0][:-1].replace('.', '_')
        labels_edit.append(f'edit_rsn_{y}')

for x in labels_edit:
    if 'X' in x:
...
@app.callback(
    Output('rsn_edit2_null_div', 'children'),
    Input('save_edit2_rsn_to_db', 'n_clicks'),
    State('vers_edit', 'children'),
    [State(key, 'value') for key in labels_edit],
    prevent_initial_call=True)
def edit_2(clicks, vers, *args):
    if clicks is None:
        return ''
    else:
        new_dict_to_db = {}
