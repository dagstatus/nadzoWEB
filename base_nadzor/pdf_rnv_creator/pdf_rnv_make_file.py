...
class CreatePdfClass:
    ...
    def make_razr_pdf(self):
        ...
        self.pdf.multi_cell(w=w_pril, h=4, align='R', border=0,
                            txt='Приложение N 2 к приказу Министерства строительства и жилищно-коммунального хозяйства Российской Федерации от 3 июня 2022 года N 446/пр')

        self.pdf.set_xy(0.0, start_head_text + 62.5)
        self.pdf.set_font('PTSerifBold', '', 16)
        self.pdf.cell(w=210.0, h=40.0, align='C', txt='РАЗРЕШЕНИЕ НА ВВОД ОБЪЕКТА В ЭКСПЛУАТАЦИЮ')
        ...
        data_table = [
            ...
            ['1.1. Дата разрешения на ввод объекта в эксплуатацию:', INPUT_DATA.get('1.1')],
            ['1.2. Номер разрешения на ввод объекта в эксплуатацию:', INPUT_DATA.get('1.2')],
            ...
            ['5.2. Номер разрешения на строительство:', INPUT_DATA.get("5.2")],
            ['5.3. Наименование органа (организации), выдавшего разрешение на строительство:', INPUT_DATA.get("5.3")],
            ...
            ['6.1. Наименование объекта капитального строительства, предусмотренного проектной документацией:',
             INPUT_DATA.get("6.1")],
            ['6.1.1. Вид объекта капитального строительства:', INPUT_DATA.get("6.1.1")],
            ['6.1.2. Назначение объекта:', INPUT_DATA.get("6.1.2")],
            ['6.1.4. Площадь застройки (кв. м):', INPUT_DATA.get('6.1.4')],
            ['6.1.4.1. Площадь застройки части объекта капитального строительства (кв. м):', INPUT_DATA.get('6.1.4.1')],
            ['6.1.5. Площадь (кв. м):', INPUT_DATA.get('6.1.5')],
            ['6.1.5.1. Площадь части объекта капитального строительства (кв. м) :', INPUT_DATA.get('6.1.5.1')],
            ['6.1.6. Площадь нежилых помещений (кв. м):', INPUT_DATA.get('6.1.6')],
            ['6.1.7. Общая площадь жилых помещений (с учетом балконов, лоджий, веранд и террас) (кв. м):',
             INPUT_DATA.get('6.1.7')],
            ['6.1.8. Количество помещений (штук):', INPUT_DATA.get('6.1.8')],
            ['6.1.9. Количество нежилых помещений (штук):', INPUT_DATA.get('6.1.9')],
            ['6.1.10. Количество жилых помещений (штук):', INPUT_DATA.get('6.1.10')],
            ['6.1.11. в том числе квартир (штук):', INPUT_DATA.get('6.1.11')],
            ['6.1.12. Количество машино-мест (штук):', INPUT_DATA.get('6.1.12')],
            ['6.1.13. Количество этажей:', INPUT_DATA.get('6.1.13')],
            ['6.1.14. в том числе, количество подземных этажей:', INPUT_DATA.get('6.1.14')],
            ['6.1.15. Вместимость (человек):', INPUT_DATA.get('6.1.15')],
            ['6.1.16. Высота (м):', INPUT_DATA.get('6.1.16')],
            ['6.1.17. Класс энергетической эффективности (при наличии):', INPUT_DATA.get('6.1.17')],
            ['6.1.18. Иные показатели :', INPUT_DATA.get('6.1.18')],
            ['6.1.19. Дата подготовки технического плана:', INPUT_DATA.get('6.1.19')],
            ['6.1.20. Страховой номер индивидуального лицевого счета кадастрового инженера, подготовившего технический план:', INPUT_DATA.get('6.1.20')],

            ['Раздел 7. Фактические показатели линейного объекта и сведения о техническом плане '],
            ['7.1. Наименование линейного объекта, предусмотренного проектной документацией:', INPUT_DATA.get("7.1")],
            ['7.1.1. Кадастровый номер реконструированного линейного объекта:', INPUT_DATA.get("7.1.1")],
            ['7.1.2. Протяженность (м) :', INPUT_DATA.get("7.1.2")],
            ['7.1.2.1. Протяженность участка или части линейного объекта (м):', INPUT_DATA.get("7.1.2.1")],
            ['7.1.3. Категория (класс):', INPUT_DATA.get("7.1.3")],
            ['7.1.4. Мощность (пропускная способность, грузооборот, интенсивность движения):', INPUT_DATA.get("7.1.4")],
            ['7.1.5. Тип (кабельная линия электропередачи, воздушная линия электропередачи, кабельно-воздушная линия электропередачи), уровень напряжения линий электропередачи:', INPUT_DATA.get("7.1.5")],
            ['7.1.6. Иные показатели :', INPUT_DATA.get("7.1.6")],
            ['7.1.7. Дата подготовки технического плана:', INPUT_DATA.get("7.1.8")],
            ['7.1.8. Страховой номер индивидуального лицевого счета кадастрового инженера, подготовившего технический план:', INPUT_DATA.get("7.1.8")]
        ]
        ...
        file_name_result = f"{INPUT_DATA.get('1.2')}_ВВОД.pdf"
