...
class WriteDB:
    ...
    def add_rsn_to_sql(self, dict_to_add: dict):
        ...
        try:
            ...
            df = pd.read_sql_query('select * from RSN', con=self.engine)
    ...
    def read_rsn_by_uid(self, uid_read):
        df = pd.read_sql(f"select * from RSN where uid='{uid_read}'", con=self.engine)
        return df
